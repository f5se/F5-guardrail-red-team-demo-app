...
Dataset Test 用于**批量将数据集中的用户 Prompt 逐条送往 F5 Guardrail**，统计护栏对每条样本的放行/拦截结果，并导出结果 CSV，进行「拦截率」或「误拦率」实测。

#### 5.1 基本流程（Step 1～5）

1. **Step 1 · 上传与测试类型**  
   填写任务名称，选择 **拦截率测试**（攻击/违规样本应被拦）或 **误拦率测试**（良性样本应放行）。选择 `.csv` / `.xlsx` 文件后点击上传。

2. **Step 2 · 数据与连接**  
   指定 Prompt 所在列（可在预览中点击对应列）、是否有表头、测试行范围；按需填写 Project / Provider / API Key，如没有这些参数，则留空使用系统缺省设定。下一步保存配置。如果有补测情况发生，且需要修改相关参数，记得修改后点下一步来保存。

3. **Step 3～4 · 确认并运行**  
   确认摘要后启动任务；运行中可查看进度、暂停、取消；完成后进入结果与下载。
...
在 **History** 中可查看过当前或往任务，误拦率任务在列表中有浅色区分；可下载原始上传文件与 **result** 结果 CSV。运行中或暂停的任务可从历史进入以继续查看或操作。

#### 5.3 注意

- 全局对「同时运行/排队」的 Dataset 任务数有上限，超额时无法新建上传，需待其它任务结束或取消后再试。  
- 结果 CSV 中的 **`guardrail_status`**（如 `ok` / `rejected` / `error`）用于汇总统计；**`label`** 列为人类可读的预期/非预期说明，请与 Step 1 所选测试类型对照理解。
