/**
 * NJS filter for chat completion with F5 Guardrail.
 * When scan outcome is "redacted", uses redactedInput from the API response
 * (at response root) to replace the last message content before forwarding to provider.
 */
function filterChatCompletion(r) {
    ...
    function scanWithAIGR(input, scanType, callback) {
        ...
        r.subrequest("/aigr_scan", {
            ...
            if (outcome === "flagged") {
                ...
                // redactedInput is at response root (sibling of "result", "id")
                ...
                if (redactedValue !== undefined && redactedValue !== null && redactedValue !== "") {
                    ...
                    try {
                        ...
                        r.error("Failed to re-parse request body for redacted replacement: " + parseErr);
                        callback();
                        return;
                    }
                    if (modifiedBody.messages && Array.isArray(modifiedBody.messages) && modifiedBody.messages.length > 0) {
                        ...
                        debugLog("Request scan redacted: using redactedInput for last message content");
                        callback(modifiedBodyStr);
                        return;
                    }
                }
                r.log("Request scan redacted but redactedInput missing or empty, forwarding original request");
                callback();
            } else {
    ...
    // optionalRequestBody: if provided (e.g. redacted body), use it; otherwise use r.requestText
    function sendToProvider(optionalRequestBody) {
        let bodyToSend = (optionalRequestBody !== undefined && optionalRequestBody !== null && optionalRequestBody !== "") ? optionalRequestBody : r.requestText;
        debugLog("Sending to LLM Provider: " + bodyToSend);
